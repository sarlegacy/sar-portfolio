...
const AboutMeModal: React.FC<AboutMeModalProps> = ({ isOpen, onClose }) => {
    ...
    return (
        ...
        >
            <div
                ref={modalRef}
                className="bg-white/80 dark:bg-mono-dark/90 backdrop-blur-lg rounded-2xl border border-gray-200 dark:border-mono-mid w-full max-w-2xl shadow-2xl shadow-black/50 relative transition-colors duration-500 focus:outline-none animate-stagger-in"
                onClick={(e) => e.stopPropagation()} // Prevent closing modal when clicking inside
                tabIndex={-1} // Make the modal container programmatically focusable
            >
                <button
                    onClick={onClose}
                ...
                <div className="p-8 sm:p-10 max-h-[80vh] overflow-y-auto">
                    <div className="flex flex-col sm:flex-row items-center gap-6 mb-6 text-center sm:text-left">
                        ...
                        <div>
                            <h2 id="about-me-title" className="font-display text-3xl font-bold text-gray-900 dark:text-white transition-colors duration-500">{profileData.name}</h2>
                    ...
                    <div className="text-gray-600 dark:text-mono-light space-y-4 whitespace-pre-line text-sm sm:text-base transition-colors duration-500 leading-relaxed">
