...
const ExperienceItem: React.FC<ExperienceItemProps> = ({ item }) => {
    return (
        <div className="relative group">
            {/* Timeline Node */}
            <div className="absolute -left-[2.1rem] top-1 w-4 h-4 bg-gray-200 dark:bg-mono-mid rounded-full border-2 border-gray-400 dark:border-mono-light transition-all duration-300 group-hover:bg-brand-green dark:group-hover:bg-brand-green group-hover:scale-150 group-hover:shadow-[0_0_20px_rgba(0,0,0,0.15)] dark:group-hover:shadow-[0_0_20px_#A0DB24]"></div>
            <div className="bg-white dark:bg-mono-dark p-6 rounded-2xl border border-gray-200 dark:border-mono-mid transition-all duration-300 hover:border-gray-300 dark:hover:border-mono-light/30 hover:shadow-[0_0_20px_rgba(0,0,0,0.05)] dark:hover:shadow-[0_0_20px_rgba(160,219,36,0.1)] hover:-translate-y-1 hover:scale-[1.02]">
                <div className="flex justify-between items-start mb-2 flex-col sm:flex-row">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="font-display text-xl font-bold text-gray-900 dark:text-white transition-colors duration-500">{item.role}</h3>
                            <p className="font-display text-md font-semibold text-gray-800 dark:text-white transition-colors duration-500">{item.company}</p>
                        </div>
                    </div>
